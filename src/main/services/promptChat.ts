  ...
  try {
    ...
    for await (const event of stream) {
      if (event.type === 'content_block_delta' && event.delta.type === 'text_delta') {
        ...
        fullContent += token
        mainWindow.webContents.send('promptChat:token', { sessionId, token })
      }
    }
    ...
    mainWindow.webContents.send('promptChat:done', { sessionId, extractedPrompt })
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err)
    mainWindow.webContents.send('promptChat:error', { sessionId, error: message })
  }
}
